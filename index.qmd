...
plot_streamfunction_anomaly(Date(2018, 12, 14))
```
...
Create a screen plot to help choose the number of PCs to retain:
...
We are trying to understand to which weather type out data point is adjacent. 

- Explain in your own words what the `update_centroids` function does and why it's important for k-means clustering.

We are figuring out what is the actual mean of the claster so we can have a mean value of the weather type. 
...
It better represents a minimum distance between two points.
...
I chose to look at 2016-12-14,weather type 6, 2017-12-14, weather type 1, and 2018-12-14, weather type 5. The first day had significuntly less negative intensity when the weather type, the second day matched really well, the third day had more intense positive corner.

- Do your EOF spatial patterns match those shown in the paper?

They match the one in the paper very well, escept for the fact that they are less smoothed out.
...
They do; however, my most common weather type is not the same as the most common type reported in the paper. This discrepancy may be due to the fact that my dataset extends through 2025, whereas the paper was published in 2015, meaning that certain patterns may have become more or less frequent over time. Additionally, I selected seven weather types instead of six, which can also shift the relative frequencies of the resulting clusters.

- What is the effect of changing the number of retained PCs on the weather type patterns?

More PC should give more noisy data. Less PC would mean that the weather types are going to be less different from each other. 

- What is the effect of rescaling PCs before clustering? Try clustering without rescaling and compare results.

In theory, our weather types should become more dominated by PC1 and PC2, since they are more influential when the PCs are not rescaled. What I found was that the number of days assigned to Weather Type 1 increased, and the overall distribution of days across weather types changed, with Types 3 through 5 shifting in their relative frequencies.

- What is the impact of changing the number of clusters (k) on the weather type patterns?

If we increase number K then weather types become too similar to each other. If we decrease the number of K from the optimal one we might miss que weather types.
